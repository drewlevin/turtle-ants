...
function buildTreeLeft(node, depth)
{
  if (depth > 0) {
    node.weight = 0;
    node.left = new Node(node, node.depth + 1, false);
    buildTreeLeft(node.left, depth-1);
...
function buildTreeRight(node, depth)
{
  if (depth > 0) {
    node.weight = 0;
    node.right = new Node(node, node.depth + 1, true);
    buildTreeRight(node.right, depth-1);
...
//    ctx.textBaseline = 'middle';
  ...
  //  }
    ...
    if (DRAW_RATE) {
      $.plot($("#flot_left"), nest.getRateSeries(false, 50), PLOT_OPTIONS);
      $.plot($("#flot_right"), nest.getRateSeries(true, 50), PLOT_OPTIONS);
    }
...
function init()
{
  ...
  buildTreeLeft(root, DEPTH);
  buildTreeRight(root, FULL_TREE_DEPTH);
