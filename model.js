...
function LymphNode(_parent, _depth)
{
  this.parent = _parent;
  this.child = null;
  this.depth = _depth;

  this.y = (HEIGHT - (DEPTH * l)) / 2 + (this.depth * l);
  this.x = 25;

  this.drawLymph = function(_ctx)
  {
    if (this.child != null)
    {
      _ctx.lineWidth = 3;
      ...
      _ctx.lineTo(this.child.x, this.child.y);
      _ctx.stroke();
      this.child.drawLymph(_ctx);
    }
...
function Ant(_dest)
{
  ...
  this.update = function() 
  {
    ...
    if (!inLymph) {
      x = dest.parent.out_x + dist * (dest.out_x - dest.parent.out_x);
      y = dest.parent.out_y + dist * (dest.out_y - dest.parent.out_y);
    } else {
      x = dest.parent.x + dist * (dest.x - dest.parent.x);
      y = dest.parent.y + dist * (dest.y - dest.parent.y);
    }
    if (!inLymph) {      
      if (dist >= 1) {
        if (dest.right != null) {
          ...
          var d = dest.depth;
          dest = lymph.child;
          for (var i=0; i < DEPTH - d; i++) {
            dest = dest.child;
          }
          lymph_ants.push(this);
          dist = 0;
          return true;
        }
        dist = 0;
...
function positionTree(node)
{
  ...
  if (node.right != null && node.left != null) {
    ...
    node.out_x = node.x + 2 * ANT_RADIUS * Math.cos(out_theta);
    node.out_y = node.y + 2 * ANT_RADIUS * Math.sin(out_theta);
    node.in_x = node.x + 2 * ANT_RADIUS * Math.cos(out_theta+Math.PI);
    node.in_y = node.y + 2 * ANT_RADIUS * Math.sin(out_theta+Math.PI);

  } else {
    node.out_x = node.x + 2 * ANT_RADIUS * Math.cos(node.theta+(Math.PI/2));
    node.out_y = node.y + 2 * ANT_RADIUS * Math.sin(node.theta+(Math.PI/2));
    node.in_x = node.x + ANT_RADIUS * Math.cos(node.theta-(Math.PI/2));
    node.in_y = node.y + ANT_RADIUS * Math.sin(node.theta-(Math.PI/2));
  }
  picker.addNode(node);
}


function buildLymph(node, depth)
{
  if (depth > 0) {
    node.child = new LymphNode(node, node.depth+1);
//    picker.addNode(node.child);
    buildLymph(node.child, depth-1);
  }
}
...
function update()
{
  ...
  for (a in lymph_ants) {
    if (lymph_ants[a].update()) {
      lymph_ants.splice(a, 1);
    }
  }
...
function init()
{
  ...
  lymph = new LymphNode(null, 0);

  buildTree(root, DEPTH);
  buildLymph(lymph, DEPTH);
